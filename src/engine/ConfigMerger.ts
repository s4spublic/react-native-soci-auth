...
/**
 * Deep-merges a partial ThemeConfig onto a base ThemeConfig.
 * Each top-level section (colors, spacing, radius, glass, motion, button)
 * is individually merged so partial overrides preserve other fields.
 */
  ...
  return {
    ...
    glass: override.glass
      ? {
          ...base.glass,
          ...override.glass,
          shadow: override.glass.shadow
            ? { ...base.glass.shadow, ...override.glass.shadow }
            : base.glass.shadow,
        }
      : base.glass,
    motion: override.motion ? { ...base.motion, ...override.motion } : base.motion,
    button: override.button ? { ...base.button, ...override.button } : base.button,
  };
}
 ...
 * Note: RN version does NOT have autoRedirect (only logging and analytics).
 */
...
export function mergeConfig(config: SociAuth_Config): ResolvedSociAuthConfig {
  ...
  const buttonSizeScale = Math.max(0.25, config.buttonSizeScale ?? 1);
  ...
  const cardTitleColor = config.cardTitleColor ?? '';
  const cardSubtitleColor = config.cardSubtitleColor ?? '';
